...
- **`Settings`** — persisted to `~/.config/com.opentypeless.app/settings.json`. Fields: `hotkey`, `auto_paste`, `polish` (PolishConfig), `history_retention_days` (u32, 0 = keep forever), `language` (Option<String>, UI language override).
...
- **`do_stop_recording`** — flips `is_recording` to false, extracts samples, resamples to 16 kHz.
- **Transcription** — `transcribe_with_cached_whisper` (loads model once, reuses WhisperContext).
...
- **`PolishConfig`** — fields: `enabled` (default true), `model` (LlamaTaiwan or Qwen25), `output_language`, `custom_prompt` (Option<String>), `mode` (PolishMode: Local or Cloud), `cloud` (CloudConfig).
- **`CloudConfig`** — fields: `provider` (CloudProvider: Groq/OpenRouter/OpenAi/Gemini/Custom), `api_key` (#[serde(skip)]), `endpoint`, `model_id` (default: "qwen/qwen3-32b").
...
- **`index.html`** — settings window with sidebar navigation. Pages: settings (hotkey, language, mic, AI polishing, model management), history (transcription log with retention settings), about, test (hotkey test mode). Calls Tauri commands via `window.__TAURI__.core.invoke`.
